...
void sign_mira_128_init_shares(sign_mira_128_shares_t *shares) {
    gfqm_vec_init(&(shares->a_), SIGN_MIRA_128_PARAM_R);
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_N_MPC; i++ ) {
        gf16_mat_init(&(shares->x[i]), 1, SIGN_MIRA_128_PARAM_K);
        gfqm_vec_init(&(shares->beta[i]), SIGN_MIRA_128_PARAM_R);
        gfqm_vec_init(&(shares->a[i]), SIGN_MIRA_128_PARAM_R);

        gf16_mat_set_zero(shares->x[i], 1, SIGN_MIRA_128_PARAM_K);
        gfqm_vec_set_zero(shares->beta[i], SIGN_MIRA_128_PARAM_R);
        gfqm_vec_set_zero(shares->a[i], SIGN_MIRA_128_PARAM_R);
...
*/
void sign_mira_128_init_main_shares(sign_mira_128_main_shares_t *main_shares) {
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++ ) {
        gf16_mat_init(&(main_shares->x[i]), 1, SIGN_MIRA_128_PARAM_K);
        gfqm_vec_init(&(main_shares->beta[i]), SIGN_MIRA_128_PARAM_R);
        gfqm_vec_init(&(main_shares->a[i]), SIGN_MIRA_128_PARAM_R);

        gf16_mat_set_zero(main_shares->x[i], 1, SIGN_MIRA_128_PARAM_K);
        gfqm_vec_set_zero(main_shares->beta[i], SIGN_MIRA_128_PARAM_R);
        gfqm_vec_set_zero(main_shares->a[i], SIGN_MIRA_128_PARAM_R);
...
void sign_mira_128_init_challenge1(sign_mira_128_challenge1_t *challenge) {
    gfqm_vec_init(&(challenge->gamma), SIGN_MIRA_128_PARAM_N);

    gfqm_vec_set_zero(challenge->gamma, SIGN_MIRA_128_PARAM_N);
...
void sign_mira_128_init_response1(sign_mira_128_response1_t *response) {
    gfqm_vec_init(&(response->alpha_), SIGN_MIRA_128_PARAM_R);
    gfqm_vec_set_zero(response->alpha_, SIGN_MIRA_128_PARAM_R);
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++ ) {
        gfqm_vec_init(&(response->alpha[i]), SIGN_MIRA_128_PARAM_R);
        gfqm_vec_set_zero(response->alpha[i], SIGN_MIRA_128_PARAM_R);
...
*/
void sign_mira_128_clear_main_shares(sign_mira_128_main_shares_t *main_shares) {
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++ ) {
...
*
* \param[out] shares sign_mira_128_shares_t Shares of all parties of the protocol
...
void sign_mira_128_compute_shares(sign_mira_128_shares_t *shares, const gf16_mat x, const gfqm_vec A, const uint8_t *theta_i, const uint8_t *salt) {
  ...
  uint8_t random[SIGN_MIRA_128_VEC_R_BYTES] = {0};
  seedexpander_shake_t seedexpander;

  uint8_t random0[2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES] = {0};
  uint8_t random1[2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES] = {0};
  uint8_t random2[2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES] = {0};
  uint8_t random3[2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES] = {0};
  uint8_t *random_x4[] = {random0, random1, random2, random3};
  
  const uint8_t *salt_x4[] = {salt, salt, salt, salt};

  gfqm_vec_set_zero(shares->a_, SIGN_MIRA_128_PARAM_R);
  gf16_mat_set_zero(shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], 1, SIGN_MIRA_128_PARAM_K);
  gfqm_vec_set_zero(shares->a[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_R);
  gfqm_vec_set_zero(shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_R);
  gfqm_set_zero(shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1]);

  // Compute the first N-1 shares
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_N_MPC; i+=4) {
    // Sample randomness
    const uint8_t *seed_x4[] = {&theta_i[SIGN_MIRA_128_SECURITY_BYTES * i], &theta_i[SIGN_MIRA_128_SECURITY_BYTES * (i + 1)],
                                &theta_i[SIGN_MIRA_128_SECURITY_BYTES * (i + 2)], &theta_i[SIGN_MIRA_128_SECURITY_BYTES * (i + 3)]};

    seedexpander_shake_init(&seedexpander, seed_x4[0], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_get_bytes(&seedexpander, random_x4[0], (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));
    seedexpander_shake_init(&seedexpander, seed_x4[1], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_get_bytes(&seedexpander, random_x4[1], (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));
    seedexpander_shake_init(&seedexpander, seed_x4[2], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_get_bytes(&seedexpander, random_x4[2], (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));
    seedexpander_shake_init(&seedexpander, seed_x4[3], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_get_bytes(&seedexpander, random_x4[3], (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));

    for(size_t j = 0; j < 4; j++) {
      if ((i + j) < (SIGN_MIRA_128_PARAM_N_MPC - 1)) {
        gfqm_vec_set_zero(shares->a[i + j], SIGN_MIRA_128_PARAM_R);
        gfqm_set_zero(shares->c[i + j]);

        // Compute share [a]_i
        gfqm_vec_from_string(shares->a[i + j], SIGN_MIRA_128_PARAM_R, &random_x4[j][0]);
        gfqm_vec_add(shares->a_, shares->a_, shares->a[i + j], SIGN_MIRA_128_PARAM_R);

        // Compute share [x]_i
        gf16_mat_from_string(shares->x[i + j], 1, SIGN_MIRA_128_PARAM_K, &random_x4[j][SIGN_MIRA_128_VEC_R_BYTES]);
        gf16_mat_add(shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->x[i + j], 1, SIGN_MIRA_128_PARAM_K);

        // Compute share [beta]_i
        gfqm_vec_from_string(shares->beta[i + j], SIGN_MIRA_128_PARAM_R, &random_x4[j][SIGN_MIRA_128_VEC_K_BYTES + SIGN_MIRA_128_VEC_R_BYTES]);
        gfqm_vec_add(shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->beta[i + j], SIGN_MIRA_128_PARAM_R);

        // Compute share [c]_i
        gfqm_from_string(shares->c[i + j], &random_x4[j][2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_VEC_K_BYTES]);
        gfqm_add(shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->c[i + j]);
      }
    }
  }

  // Sample randomness for share N
  seedexpander_shake_init(&seedexpander, &theta_i[SIGN_MIRA_128_SECURITY_BYTES * (SIGN_MIRA_128_PARAM_N_MPC - 1)], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
  seedexpander_shake_get_bytes(&seedexpander, random, SIGN_MIRA_128_VEC_R_BYTES);

  // Compute u and share [a]_N
  gfqm_vec_from_string(shares->a[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_R, &random[0]);
  gfqm_vec_add(shares->a_, shares->a_, shares->a[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_R);

  // Computing c and share [c]_N
  gfqm_vec_inner_product(tmp, shares->a_, A, SIGN_MIRA_128_PARAM_R);
  gfqm_add(shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1], tmp);

  // Compute shares [beta]_N, [x]_N
  gfqm_vec_add(shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], A, SIGN_MIRA_128_PARAM_R);
  gf16_mat_add(shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], x, 1, SIGN_MIRA_128_PARAM_K);

  memset(random, 0, SIGN_MIRA_128_VEC_R_BYTES);
}



/**
* \fn void sign_mira_commit_to_shares(uint8_t *commits, uint8_t e, const uint8_t *salt, const sign_mira_128_shares_t *shares, const uint8_t *theta_i)
*
...
*/
void sign_mira_128_commit_to_shares(uint8_t *commits, uint8_t e, const uint8_t *salt, const sign_mira_128_shares_t *shares, const uint8_t *theta_i) {
  ...
  uint8_t i = (uint8_t) ((uint16_t)SIGN_MIRA_128_PARAM_N_MPC - 1);
  ...
  gf16_mat_to_string(&state[0], shares->x[i], 1, SIGN_MIRA_128_PARAM_K);
  gfqm_vec_to_string(&state[SIGN_MIRA_128_VEC_K_BYTES], shares->beta[i], SIGN_MIRA_128_PARAM_R);
  gfqm_to_string(&state[SIGN_MIRA_128_VEC_K_BYTES + SIGN_MIRA_128_VEC_R_BYTES], shares->c[i]);

  hash_sha3_absorb(&ctx, state, SIGN_MIRA_128_PARAM_STATE_BYTES);
  hash_sha3_finalize(&commits[2 * SIGN_MIRA_128_SECURITY_BYTES * i], &ctx);

  memset(state, 0, SIGN_MIRA_128_PARAM_STATE_BYTES);
}
...
* \param[in] shares sign_mira_shares_t Representation of the shares
*/
void sign_mira_128_compute_main_shares(sign_mira_128_main_shares_t *main_shares, const sign_mira_128_shares_t *shares) {

    uint8_t bit = 0x1;
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
        for(size_t j = 0; j < SIGN_MIRA_128_PARAM_N_MPC; j++) {
            if((j & bit) == 0) {
                gf16_mat_add(main_shares->x[i], main_shares->x[i], shares->x[j], 1, SIGN_MIRA_128_PARAM_K);
                gfqm_vec_add(main_shares->beta[i], main_shares->beta[i], shares->beta[j], SIGN_MIRA_128_PARAM_R);
                gfqm_vec_add(main_shares->a[i], main_shares->a[i], shares->a[j], SIGN_MIRA_128_PARAM_R);
...
void sign_mira_128_compute_challenge1(sign_mira_128_challenge1_t *challenge, const uint8_t *seed_input, const uint8_t *salt) {
  ...
  uint8_t random[SIGN_MIRA_128_VEC_N_PLUS_ONE_BYTES] = {0};
  seedexpander_shake_t seedexpander;
  seedexpander_shake_init(&seedexpander, seed_input, 2 * SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);

  gfqm_vec tmp;
  gfqm_vec_init(&tmp, SIGN_MIRA_128_PARAM_N + 1);

  for(size_t e = 0; e < SIGN_MIRA_128_PARAM_TAU; e++) {
    seedexpander_shake_get_bytes(&seedexpander, random, SIGN_MIRA_128_VEC_N_PLUS_ONE_BYTES);
    gfqm_vec_from_string(tmp, SIGN_MIRA_128_PARAM_N + 1, random);
    gfqm_vec_set(challenge[e].gamma, tmp, SIGN_MIRA_128_PARAM_N);
    gfqm_set(challenge[e].epsilon, tmp[SIGN_MIRA_128_PARAM_N]);
  }

  gfqm_vec_clear(tmp);
  memset(random, 0, SIGN_MIRA_128_VEC_N_PLUS_ONE_BYTES);
}
...
void sign_mira_128_compute_response1(sign_mira_128_response1_t *response, const sign_mira_128_main_shares_t *shares, const sign_mira_128_challenge1_t *challenge, const gf16_mat M0, const gf16_mat *Mi, const gf16_mat x, const gfqm_vec a) {
    ...
    gf16_mat Ei, tmp;
    gf16_mat_init(&Ei, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
    gf16_mat_init(&tmp, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);

    gfqm_vec Ei_vec;
    gfqm_vec Ei_vec_pow_k;
    gfqm_vec_init(&Ei_vec, SIGN_MIRA_128_PARAM_N);
    gfqm_vec_init(&Ei_vec_pow_k, SIGN_MIRA_128_PARAM_N);

    gfqm z[SIGN_MIRA_128_PARAM_D];
    gfqm gamma_e;
    gfqm_vec w;
    gfqm_vec_init(&w, SIGN_MIRA_128_PARAM_R);

    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
        // Compute [E]
        // M0 on leaf 0 => present in every main share
        gf16_mat_set(Ei, M0, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);

        for(int j=0 ; j<SIGN_MIRA_128_PARAM_K ; j++) {
            gf16_mat_scalar_mul(tmp, Mi[j], shares->x[i][j], SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
            gf16_mat_add(Ei, Ei, tmp, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
        }

        //"Transpose" into elements of Fqm
        for(int j=0 ; j<SIGN_MIRA_128_PARAM_N ; j++) {
            for(int k=0 ; k<SIGN_MIRA_128_PARAM_M ; k++) {
                Ei_vec[j][k] = Ei[k*SIGN_MIRA_128_PARAM_N + j];
            }
            gfqm_set(Ei_vec_pow_k[j], Ei_vec[j]);
        }

        // Compute [w] and [z]
        for(size_t k=0 ; k < SIGN_MIRA_128_PARAM_R ; k++) {
            gfqm_set_zero(w[k]);
            //w[k] = <gamma . (e_ji^q^k)>
            for(int j=0 ; j<SIGN_MIRA_128_PARAM_N ; j++) {
            ...
            for(size_t j = 0; j < SIGN_MIRA_128_PARAM_N; j++) {
        ...
        // Compute [alpha]
        gfqm_vec_scalar_mul(response->alpha[i], w, challenge->epsilon, SIGN_MIRA_128_PARAM_R);
        gfqm_vec_add(response->alpha[i], response->alpha[i], shares->a[i], SIGN_MIRA_128_PARAM_R);
    }

    // Compute alpha
    sign_mira_128_reconstruct_alpha(response, M0, Mi, x, a, challenge, 1);

    // Compute [v]
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
        gfqm_mul(response->v[i], challenge->epsilon, z[i]);
        gfqm_vec_inner_product(gamma_e, response->alpha_, shares->beta[i], SIGN_MIRA_128_PARAM_R); //Use gamma_e as a tmp buffer
...
void sign_mira_128_reconstruct_alpha(sign_mira_128_response1_t *response, const gf16_mat M0, const gf16_mat *Mi, const gf16_mat x, const gfqm_vec a, const sign_mira_128_challenge1_t *challenge, int include_M0) {
    gfqm_vec w, aux;
    gfqm_vec_init(&w, SIGN_MIRA_128_PARAM_R);
    gfqm_vec_init(&aux, SIGN_MIRA_128_PARAM_R);
    ...
    //Recompute E
    if(include_M0 != 0) gf16_mat_set(E, M0, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
    else gf16_mat_set_zero(E, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);

    for(int i=0 ; i<SIGN_MIRA_128_PARAM_K ; i++) {
        gf16_mat_scalar_mul(tmp, Mi[i], x[i], SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
        gf16_mat_add(E, E, tmp, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
    }

    //"Transpose" into elements of Fqm
    gfqm_vec E_vec;
    gfqm_vec E_vec_pow_k;
    gfqm_vec_init(&E_vec, SIGN_MIRA_128_PARAM_N);
    gfqm_vec_init(&E_vec_pow_k, SIGN_MIRA_128_PARAM_N);

    for(int j=0 ; j<SIGN_MIRA_128_PARAM_N ; j++) {
        for(int k=0 ; k<SIGN_MIRA_128_PARAM_M ; k++) {
            E_vec[j][k] = E[k*SIGN_MIRA_128_PARAM_N + j];
        }
    ...
    // Compute [alpha]_ch2
    gfqm_vec_set_zero(response->alpha_, SIGN_MIRA_128_PARAM_R);
    gfqm_vec_scalar_mul(response->alpha_, w, challenge->epsilon, SIGN_MIRA_128_PARAM_R);
    gfqm_vec_add(response->alpha_, response->alpha_, a, SIGN_MIRA_128_PARAM_R);
...
void sign_mira_128_reconstruct_share(gf16_mat x, gfqm_vec a, const uint8_t *theta_i, uint8_t challenge2, const uint8_t *salt) {
  ...
  uint8_t random[2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES] = {0};
  seedexpander_shake_t seedexpander;

  // Sample randomness from seed theta
  seedexpander_shake_init(&seedexpander, &theta_i[SIGN_MIRA_128_SECURITY_BYTES * challenge2], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
  seedexpander_shake_get_bytes(&seedexpander, random, 2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES);

  // Recompute shares [a], [x] of index challenge2
  gfqm_vec_from_string(a, SIGN_MIRA_128_PARAM_R, &random[0]);

  // Compute share [x]_i
  gf16_mat_from_string(x, 1, SIGN_MIRA_128_PARAM_K, &random[SIGN_MIRA_128_VEC_R_BYTES]);

  memset(random, 0, (2 * SIGN_MIRA_128_VEC_R_BYTES + SIGN_MIRA_128_PARAM_M_BYTES + SIGN_MIRA_128_VEC_K_BYTES));
}
...
*/
void sign_mira_128_recompute_commitments(uint8_t *commits, uint8_t *theta_i, uint8_t e, const uint8_t *salt, const uint8_t *state, uint8_t hidden) {

  uint8_t domain_separator = DOMAIN_SEPARATOR_COMMITMENT;
  sign_mira_128_seed_tree_node_t partial_tree_seeds[SIGN_MIRA_128_PARAM_N_MPC_LOG2] = {0};
  sign_mira_128_seed_tree_t theta_tree = {0};

  memcpy(&partial_tree_seeds, &state[0], SIGN_MIRA_128_PARAM_TREE_PATH_BYTES);
  sign_mira_128_tree_expand_partial(theta_tree, (const sign_mira_128_seed_tree_node_t *) partial_tree_seeds, salt, hidden);
  memcpy(theta_i, &theta_tree[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_TREE_LEAF_BYTES);
  ...
  memcpy(&commits[2 * SIGN_MIRA_128_SECURITY_BYTES * hidden], &state[SIGN_MIRA_128_PARAM_TREE_PATH_BYTES], 2 * SIGN_MIRA_128_SECURITY_BYTES);

  // Recompute commitment N
  uint8_t i = (uint8_t) ((uint16_t)SIGN_MIRA_128_PARAM_N_MPC - 1);

  if (i == hidden) {
    memcpy(&commits[2 * SIGN_MIRA_128_SECURITY_BYTES * i], &state[SIGN_MIRA_128_PARAM_TREE_PATH_BYTES], 2 * SIGN_MIRA_128_SECURITY_BYTES);
  }
  else {
    hash_sha3_ctx ctx;
    hash_sha3_init(&ctx);
    ...
    hash_sha3_absorb(&ctx, &i, sizeof(uint8_t));
    hash_sha3_absorb(&ctx, &theta_i[SIGN_MIRA_128_SECURITY_BYTES * i], SIGN_MIRA_128_SECURITY_BYTES);
    hash_sha3_absorb(&ctx, &state[SIGN_MIRA_128_PARAM_TREE_PATH_BYTES + (2 * SIGN_MIRA_128_SECURITY_BYTES) + SIGN_MIRA_128_VEC_R_BYTES], SIGN_MIRA_128_PARAM_STATE_BYTES);
    hash_sha3_finalize(&commits[2 * SIGN_MIRA_128_SECURITY_BYTES * i], &ctx);
  }
}
...
* \param[in] salt String containing the salt
*/
void sign_mira_128_recompute_shares(sign_mira_128_shares_t *shares, const uint8_t *theta_i, const uint8_t *state, uint8_t second_challenge, const uint8_t *salt) {
  ...
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_N_MPC; i+=4) {
    ...
    for(size_t j = 0; j < 4; j++) {
      if (((i + j) < (SIGN_MIRA_128_PARAM_N_MPC - 1)) && ((i + j) != second_challenge)) {
  ...
  // Recompute share N
  if ((SIGN_MIRA_128_PARAM_N_MPC - 1) != second_challenge) {
    // Sample randomness
    seedexpander_shake_init(&seedexpander, &theta_i[SIGN_MIRA_128_SECURITY_BYTES * (SIGN_MIRA_128_PARAM_N_MPC - 1)], SIGN_MIRA_128_SECURITY_BYTES, salt, 2 * SIGN_MIRA_128_SECURITY_BYTES);
    seedexpander_shake_get_bytes(&seedexpander, random, SIGN_MIRA_128_VEC_R_BYTES);
  ...
  // Recompute [x]_N
  gf16_mat_from_string(shares->x[SIGN_MIRA_128_PARAM_N_MPC - 1], 1, SIGN_MIRA_128_PARAM_K, &state[0]);

  // Recompute [beta]_N
  gfqm_vec_from_string(shares->beta[SIGN_MIRA_128_PARAM_N_MPC - 1], SIGN_MIRA_128_PARAM_R, &state[SIGN_MIRA_128_VEC_K_BYTES]);

  // Recompute [c]_N
  gfqm_from_string(shares->c[SIGN_MIRA_128_PARAM_N_MPC - 1], &state[SIGN_MIRA_128_VEC_K_BYTES + SIGN_MIRA_128_VEC_R_BYTES]);
...
* \param[in] second_challenge Integer concerning the second challenge
*/
void sign_mira_128_recompute_main_shares(sign_mira_128_main_shares_t *main_shares, const sign_mira_128_shares_t *shares, uint8_t second_challenge) {
...
* \param[in] shares sign_mira_128_shares_t representation of shares
* \param[in] second_challenge Integer concerning the second challenge
*/
void sign_mira_128_recompute_additional_main_share(gf16_mat x, gfqm_vec beta, gfqm_vec a, gfqm c, const sign_mira_128_shares_t *shares, uint8_t second_challenge) {
  ...
  gf16_mat_set_zero(x, 1, SIGN_MIRA_128_PARAM_K);
  gfqm_vec_set_zero(beta, SIGN_MIRA_128_PARAM_R);
  gfqm_vec_set_zero(a, SIGN_MIRA_128_PARAM_R);
  gfqm_set_zero(c);

  for(size_t j = 0; j < SIGN_MIRA_128_PARAM_N_MPC; j++) {
    if (j != second_challenge) {
      if ((j & 0x1) != 0) {
        gf16_mat_add(x, x, shares->x[j], 1, SIGN_MIRA_128_PARAM_K);
        gfqm_vec_add(beta, beta, shares->beta[j], SIGN_MIRA_128_PARAM_R);
        gfqm_vec_add(a, a, shares->a[j], SIGN_MIRA_128_PARAM_R);
...
void sign_mira_128_add_alpha_ch2(sign_mira_128_response1_t *response, const uint8_t *state, uint8_t second_challenge) {
  ...
  gfqm_vec_from_string(response->alpha_, SIGN_MIRA_128_PARAM_R, state);

  uint8_t bit = 0x1;
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
    if((second_challenge & bit) == 0) {
      gfqm_vec_add(response->alpha[i], response->alpha[i], response->alpha_, SIGN_MIRA_128_PARAM_R);
    }
    bit <<= 1;
  }
}
...
void sign_mira_128_recompute_response1(sign_mira_128_response1_t *response, const sign_mira_128_main_shares_t *shares, const sign_mira_128_challenge1_t *challenge, const gf16_mat x, const gfqm_vec beta, const gfqm_vec a, const gfqm c, const gf16_mat M0, const gf16_mat* Mi, uint8_t second_challenge) {
  ...
  gfqm_vec_init(&alpha, SIGN_MIRA_128_PARAM_R);
  gfqm_vec_init(&aux, SIGN_MIRA_128_PARAM_R);
  gfqm_vec_init(&w, SIGN_MIRA_128_PARAM_R);
  gfqm_vec_init(&beta_c, SIGN_MIRA_128_PARAM_R);
  gfqm_vec_init(&beta_t, SIGN_MIRA_128_PARAM_R);

  gf16_mat_init(&Ei, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
  gf16_mat_init(&tmp, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
  gf16_mat_init(&x_, 1, SIGN_MIRA_128_PARAM_K);
  gfqm_vec_init(&Ei_vec, SIGN_MIRA_128_PARAM_N);
  gfqm_vec_init(&Ei_vec_pow_k, SIGN_MIRA_128_PARAM_N);

  // Compute [z], [beta] and [c] for everything except missing leaf (z_c, beta_c, c_c)
  // Compute [E]
  if(second_challenge == 0) gf16_mat_set_zero(Ei, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
  else gf16_mat_set(Ei, M0, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
  gf16_mat_add(x_, x, shares->x[0], 1, SIGN_MIRA_128_PARAM_K);

  for(int j=0 ; j<SIGN_MIRA_128_PARAM_K ; j++) {
      gf16_mat_scalar_mul(tmp, Mi[j], x_[j], SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
      gf16_mat_add(Ei, Ei, tmp, SIGN_MIRA_128_PARAM_M, SIGN_MIRA_128_PARAM_N);
  }
  ...
  gfqm_vec_add(beta_c, beta, shares->beta[0], SIGN_MIRA_128_PARAM_R);
  ...
  for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
    ...
    // Compute [alpha]
    gfqm_vec_scalar_mul(aux, w, challenge->epsilon, SIGN_MIRA_128_PARAM_R);
    gfqm_vec_add(aux, aux, shares->a[i], SIGN_MIRA_128_PARAM_R);
    gfqm_vec_add(response->alpha[i], response->alpha[i], aux, SIGN_MIRA_128_PARAM_R);

    bit <<= 1;
  }

    // Compute plain alpha
    gfqm_vec_set(alpha, response->alpha_, SIGN_MIRA_128_PARAM_R);
    sign_mira_128_reconstruct_alpha(response, M0, Mi, x, a, challenge, 0);

    if ((second_challenge & 0x1) != 0) {
      gfqm_vec_add(response->alpha_, response->alpha_, alpha, SIGN_MIRA_128_PARAM_R);
    }
    gfqm_vec_add(response->alpha_, response->alpha_, response->alpha[0], SIGN_MIRA_128_PARAM_R);
    ...
    for(size_t i = 0; i < SIGN_MIRA_128_PARAM_D; i++) {
      if ((second_challenge & bit) == 0) {
        ...
        gfqm_vec_add(beta_t, beta_c, shares->beta[i], SIGN_MIRA_128_PARAM_R);
        gfqm_add(c_t, c_c, shares->c[i]);
      }
      else {
        gfqm_vec_set(beta_t, shares->beta[i], SIGN_MIRA_128_PARAM_R);
        gfqm_set(c_t, shares->c[i]);
      }

      gfqm_mul(response->v[i], challenge->epsilon, z[i]);
      gfqm_vec_inner_product(gamma_e, response->alpha_, beta_t, SIGN_MIRA_128_PARAM_R);
      gfqm_add(response->v[i], response->v[i], gamma_e);
